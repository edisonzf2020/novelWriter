...
- [ ] Task 1: 设计统一工具基类和Schema系统 (AC: 3)
  - [ ] 创建`BaseTool`抽象基类，定义标准工具接口
  - [ ] 使用Pydantic v2实现参数验证Schema
  - [ ] 设计统一的工具元数据管理（name, description, parameters）
  - [ ] 实现工具权限检查装饰器集成
  - [ ] 添加工具执行时间监控装饰器

- [ ] Task 2: 实现项目信息工具 (AC: 1, 4)
  - [ ] 创建`ProjectInfoTool`类，基于`NovelWriterAPI.getProjectMeta()`
  - [ ] 实现项目基本信息获取（标题、作者、字数统计）
  - [ ] 添加项目设置信息访问
  - [ ] 实现结果标准化格式化
  - [ ] 确保直接调用延迟<5ms

- [ ] Task 3: 实现文档操作工具集 (AC: 1, 2, 4)
  - [ ] 创建`DocumentListTool`获取文档列表
  - [ ] 创建`DocumentReadTool`读取文档内容
  - [ ] 创建`DocumentWriteTool`写入文档内容（包含权限验证）
  - [ ] 创建`CreateDocumentTool`创建新文档
  - [ ] 所有工具基于对应的NovelWriterAPI方法，保证性能

- [ ] Task 4: 实现项目结构工具 (AC: 1, 4)
  - [ ] 创建`ProjectTreeTool`获取完整项目树结构
  - [ ] 实现层级结构的标准化表示
  - [ ] 添加节点类型信息（文档、文件夹、根等）
  - [ ] 支持过滤器参数（按类型、状态等）

- [ ] Task 5: 实现搜索功能工具 (AC: 1, 2)
  - [ ] 创建`GlobalSearchTool`基于`NovelWriterAPI.searchProject()`
  - [ ] 支持全文搜索、标题搜索、标签搜索
  - [ ] 实现搜索结果的标准化格式
  - [ ] 添加搜索性能优化，确保大项目搜索<10ms

- [ ] Task 6: 实现标签和统计工具 (AC: 1, 4)
  - [ ] 创建`TagListTool`获取项目标签列表
  - [ ] 创建`ProjectStatsTool`获取项目统计信息
  - [ ] 实现标签分类和使用频率统计
  - [ ] 添加实时统计计算优化

- [ ] Task 7: 工具注册和发现系统 (AC: 3)
  - [ ] 扩展Story 1.3中的`ToolRegistry`支持本地工具自动发现
  - [ ] 实现工具动态注册机制
  - [ ] 添加工具元数据验证和冲突检测
  - [ ] 集成工具健康检查和状态监控

- [ ] Task 8: 性能优化和测试验证 (AC: 2, 5)
  - [ ] 创建性能基准测试，验证所有工具P95延迟<10ms
  - [ ] 实现工具调用结果与直接API调用的一致性测试
  - [ ] 添加并发调用测试验证线程安全性
  - [ ] 建立性能回归测试套件
