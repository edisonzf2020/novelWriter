...
from novelwriter.api.tools.base import ToolPermission
...
class LocalToolWrapper:
    ...
    def createToolHandler(
        self,
        api_method: str,
        transform_params: Callable | None = None,
    ...
    def _getProjectInfo(self, include_stats: bool = False) -> dict[str, Any]:
        ...
        info = dict(self._nw_api.getProjectMeta())

        if include_stats:
            stats = self._nw_api.getProjectStatistics()
            info["statistics"] = dict(stats)
    ...
    def _listDocuments(self, scope: str) -> list[dict[str, Any]]:
        ...
        return [dict(doc) if isinstance(doc, dict) else doc for doc in documents]
    ...
    def _readDocument(self, item_handle: str, include_metadata: bool = False) -> dict[str, Any]:
        ...
        if include_metadata:
            try:
                ...
                result["metadata"] = dict(meta) if meta else {}
        ...
        try:
            for result in results:
                if isinstance(result, dict):
                    ...
                    formatted_results.append({
                        "item_handle": str(result),
                        "title": "",
                        "matches": []
                    })
        except (TypeError, AttributeError):
            # Handle case where results is a Mock or not iterable
            formatted_results = []
